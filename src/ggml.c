...
inline static void ggml_vec_sin_cos_f32  (const int n, float * y, const float * x) {
    for (int i = 0; i < n; ++i) y[i] = (i % 2 == 0)? sinf(x[i]) : cosf(x[i]);
}
inline static void ggml_vec_cos_sin_f32  (const int n, float * y, const float * x) {
    for (int i = 0; i < n; ++i) y[i] = (i % 2 == 0)? cosf(x[i]) : sinf(x[i]);
}
...
static const char * GGML_OP_SYMBOL[GGML_OP_COUNT] = {
    ...
    "max(x, dim)"
    "min(x, dim)"
    "corr(x1, x2, patch_size)"
    "global_attn(x, dim1, dim2)"
    "argmax(x)",
    "argmax_ext(x, dim)",
    "repeat(x)",
    "repeat_back(x)",
    "repeat_ext(x, n0, n1, n2, n3)",
    "concat(x, y)",
    "cat(n, ...)",
    "silu_back(x)",
    "norm(x)",
    ...
    "flip(x, dim)",
    "scatter(x, dim, index)",
    "slice_scatter(x, e, dim, start, stop, step)",
    "rftt2(x)",
    "irfft2(x)",
    "pad(x)",
    "replicaition_pad2d(x, left, right, top, bottom)",
    "reflection_pad2d(x, left, right, top, bottom)",
    "deconv_pad2d(x, stride)",
...
static struct ggml_tensor * ggml_new_tensor_impl(
    ...
    if (obj_new == NULL) {
        printf("create new tensor failed: type = %d, ne: %ld x %ld x %ld x %ld", type, ne[0], ne[1], ne[2], ne[3]);
    }
    GGML_ASSERT(obj_new);
...
static struct ggml_tensor * ggml_sin_cos_impl(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        bool inplace) {
    bool is_node = false;
...
static struct ggml_tensor * ggml_cos_sin_impl(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        bool inplace) {
    bool is_node = false;
...
struct ggml_tensor * ggml_sin_cos(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_sin_cos_impl(ctx, a, false);
}
struct ggml_tensor * ggml_sin_cos_inplace(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_sin_cos_impl(ctx, a, true);
}
struct ggml_tensor * ggml_cos_sin(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_cos_sin_impl(ctx, a, false);
}
struct ggml_tensor * ggml_cos_sin_inplace(
        struct ggml_context * ctx,
        struct ggml_tensor  * a) {
    return ggml_cos_sin_impl(ctx, a, true);
}
...
// dell_xxxx
struct ggml_tensor * ggml_mean_ext(
        struct ggml_context * ctx,
        struct ggml_tensor * a,
        int dim) {
    bool is_node = false;
...
// dell_xxxx
struct ggml_tensor * ggml_corr(
        struct ggml_context * ctx,
        struct ggml_tensor * f1,
        struct ggml_tensor * f2,
        int patch_size) {
    bool is_node = false;
    GGML_ASSERT(ggml_are_same_shape(f1, f2));
    ...
    int64_t ne[4] = { f1->ne[0], f1->ne[1], f1->ne[2], f1->ne[3] };
    ne[2] = patch_size * patch_size; // on channel 
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
    ggml_set_op_params_i32(result, 0, patch_size);

    result->op   = GGML_OP_CORR;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = f1;
    result->src[1] = f2;
...
// dell_xxxx
struct ggml_tensor * ggml_global_attn(
        struct ggml_context * ctx,
        ...
        int max_disp) {
    bool is_node = false;
    GGML_ASSERT(ggml_is_contiguous(local_attn));
    ...
    int64_t ne[4] = { h2*w2, h1*w1, 1, 1 };
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, ne);
    ggml_set_op_params_i32(result, 0, h1);
    ggml_set_op_params_i32(result, 1, w1);
    ggml_set_op_params_i32(result, 2, h2);
    ggml_set_op_params_i32(result, 3, w2);
    ggml_set_op_params_i32(result, 4, max_disp);

    result->op = GGML_OP_GLOBAL_ATTN;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = local_attn;
...
struct ggml_tensor * ggml_repeat_ext(
    ...
    ggml_set_op_params_i32(result, 3, n3);

    result->op   = GGML_OP_REPEAT_EXT;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
...
struct ggml_tensor * ggml_cat(
    ...
    int n, ...) {
    GGML_ASSERT(n > 0);
    GGML_ASSERT(n <= GGML_MAX_SRC);

    struct ggml_tensor * t[GGML_MAX_SRC];
    va_list args;
    va_start(args, n);
    for (int i = 0; i < n; ++i) {
        struct ggml_tensor *arg = va_arg(args, struct ggml_tensor *);
        GGML_ASSERT(arg->type == GGML_TYPE_F32);

        t[i] = arg; 
    }
    // get dim at last args ...
    const int dim = va_arg(args, int);
    va_end(args);
...
// dell_xxxx
static struct ggml_tensor * ggml_norm_ext_impl(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        int dim,
        float eps) {
...
struct ggml_tensor * ggml_norm_ext(
        ...
        int dim,
        float eps) {
    return ggml_norm_ext_impl(ctx, a, dim, eps);
}
...
struct ggml_tensor * ggml_reshape_2d(
    ...
    ne0 = (ne0 < 0)? ggml_nelements(a)/ne1 : ne0;
    ne1 = (ne1 < 0)? ggml_nelements(a)/ne0 : ne1;
    GGML_ASSERT(ggml_is_contiguous(a));
...
struct ggml_tensor * ggml_reshape_3d(
        ...
        int64_t               ne2) {
    // dell_xxxx
    GGML_ASSERT(ne0 != 0 && ne1 != 0 && ne2 != 0);
    ne0 = (ne0 < 0)? ggml_nelements(a)/ne1/ne2 : ne0;
    ne1 = (ne1 < 0)? ggml_nelements(a)/ne0/ne2 : ne1;
    ne2 = (ne2 < 0)? ggml_nelements(a)/ne0/ne1 : ne2;
...
struct ggml_tensor * ggml_reshape_4d(
    ...
    ne0 = (ne0 < 0)? ggml_nelements(a)/ne1/ne2/ne3 : ne0;
    ne1 = (ne1 < 0)? ggml_nelements(a)/ne0/ne2/ne3 : ne1;
    ne2 = (ne2 < 0)? ggml_nelements(a)/ne0/ne1/ne3 : ne2;
    ne3 = (ne3 < 0)? ggml_nelements(a)/ne0/ne1/ne2 : ne3;
...
// dell_xxxx
static struct ggml_tensor * ggml_softmax_impl(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        int                   dim) {
    GGML_ASSERT(ggml_is_contiguous(a));
...
// dell_xxxx
struct ggml_tensor * ggml_softmax(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        int                   dim) {
    return ggml_softmax_impl(ctx, a, dim);
}
...
struct ggml_tensor * ggml_constant(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        float                 value) {
    bool is_node = false;
...
struct ggml_tensor * ggml_add_constant(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        float                 value) {
    // dell_xxxx
    ...
    result->op   = GGML_OP_ADD_CONSTANT;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = a;
...
static struct ggml_tensor * ggml_shuffle_impl(
    ...
    const int64_t nes[4] = {
        a->ne[0] * scale_factor, a->ne[1] * scale_factor, a->ne[2]/scale_factor/scale_factor, a->ne[3]};
    struct ggml_tensor * result = ggml_new_tensor(ctx, a->type, 4, nes);

    result->op = GGML_OP_SHUFFLE;
...
static struct ggml_tensor * ggml_flip_impl(
    struct ggml_context * ctx,
    ...
    ggml_set_op_params_i32(result, 0, dim0);
    ggml_set_op_params_i32(result, 1, dim1);
    ggml_set_op_params_i32(result, 2, dim2);
    ggml_set_op_params_i32(result, 3, dim3);
...
static struct ggml_tensor * ggml_slice_scatter_impl(
    struct ggml_context * ctx,
    struct ggml_tensor * x,
    ...
    GGML_ASSERT(x->type == e->type);
    GGML_ASSERT(dim >= 0 && dim < 4);
    for (int i = 0; i < 4; i++) { // GGML_MAX_DIMS -- 4 ?
        if (i == dim)
            continue;
        GGML_ASSERT(x->ne[i] == e->ne[i]);
    }

    struct ggml_tensor * result = ggml_dup_tensor(ctx, x);

    result->op = GGML_OP_SLICE_SCATTER;
    ggml_set_op_params_i32(result, 0, dim);
    ggml_set_op_params_i32(result, 1, start);
    ggml_set_op_params_i32(result, 2, stop);
    ggml_set_op_params_i32(result, 3, step);
...
static struct ggml_tensor * ggml_rfft2_impl(
    struct ggml_context * ctx,
    struct ggml_tensor * a) {
    bool is_node = false;
    ...
    GGML_ASSERT(a->ne[1] % 2 == 0); // H % 2 == 0
    // Torch Format:
    // RFFT2: Real: (B, C, H, W) --> Complex: (B, C, H, (W/2) + 1) --> Real: (B, 2*C, H, (W/2) + 1)
    //IRFFT2: Real: (B, 2*C, H, (W/2) + 1) --> Complex: (B, C, H, (W/2+1)) --> Real: (B, C, H, W)

    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, a->type, (a->ne[0]/2) + 1, a->ne[1], 2*a->ne[2], a->ne[3]);

    result->op = GGML_OP_RFFT2;
...
static struct ggml_tensor * ggml_irfft2_impl(
    struct ggml_context * ctx,
    struct ggml_tensor * a) {
    bool is_node = false;
    ...
    GGML_ASSERT(a->ne[0] % 2 == 1 && a->ne[1] % 2 == 0 && a->ne[2] % 2 == 0); // W%2 == 1, H%2 == 0, C%2 == 0
    // Torch Format:
    // RFFT2: Real: (B, C, H, W) --> Complex: (B, C, H, (W/2)+1) --> Real: (B, 2*C, H, (W/2) + 1)
    //IRFFT2: Real: (B, 2*C, H, (W/2) + 1) --> Complex: (B, C, H, (W/2)+1) --> Real: (B, C, H, W)

    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, a->type, 2*(a->ne[0] -1), a->ne[1], a->ne[2]/2, a->ne[3]);

    result->op = GGML_OP_IRFFT2;
...
// input shape: [B, C, Hin, Win]
// grid shape: [B, Hout, Wout, 2], value range [-1.0, 1.0]
// output shape: [B, C, Hout, Wout]

struct ggml_tensor * ggml_grid_mesh(
    struct ggml_context * ctx,
    int                   B,
    int                   H,
    int                   W,
    ...
    bool is_node = false;
    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, 2, W, H, B);
    ggml_set_op_params_i32(result, 0, norm);

    result->op = GGML_OP_GRID_MESH;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
...
// dell_xxxx
struct ggml_tensor * ggml_soft_splat(
        struct ggml_context * ctx,
        struct ggml_tensor  * x,
        struct ggml_tensor  * flow)
{
    ...
    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, x->type, x->ne[0], x->ne[1], x->ne[2], x->ne[3]);
    result->op = GGML_OP_SOFT_SPLAT;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = x;
    result->src[1] = flow;
...
struct ggml_tensor * ggml_euler_motion(
        struct ggml_context * ctx,
        struct ggml_tensor  * flow,
        int                   n) {
    bool is_node = false;
    ...
    struct ggml_tensor * result = ggml_dup_tensor(ctx, flow);
    ggml_set_op_params_i32(result, 0, n);

    result->op = GGML_OP_EULER_MOTION;
    result->grad = is_node ? ggml_dup_tensor(ctx, result) : NULL;
    result->src[0] = flow;
...
struct ggml_tensor * ggml_shuffle(
    ...
    struct ggml_tensor * a,
    int scale_factor) {
    return ggml_shuffle_impl(ctx, a, scale_factor);
}
...
struct ggml_tensor * ggml_flip(
    ...
    struct ggml_tensor * x,
    int dim0, int dim1, int dim2, int dim3) {
    return ggml_flip_impl(ctx, x, dim0, dim1, dim2, dim3);
}
...
struct ggml_tensor * ggml_scatter(
    ...
    return ggml_scatter_impl(ctx, x, dim, index);
}

struct ggml_tensor * ggml_slice_scatter(
    struct ggml_context * ctx,
    struct ggml_tensor * x,
    struct ggml_tensor * e,
    int dim,
    ...
    int stop,
    int step) {
    return ggml_slice_scatter_impl(ctx, x, e, dim, start, stop, step);
}

struct ggml_tensor * ggml_rfft2(
    struct ggml_context * ctx,
    struct ggml_tensor * a) {
    return ggml_rfft2_impl(ctx, a);
}

struct ggml_tensor * ggml_irfft2(
    struct ggml_context * ctx,
    struct ggml_tensor * a) {
    return ggml_irfft2_impl(ctx, a);
}
...
struct ggml_tensor * ggml_replication_pad2d(
    struct ggml_context * ctx,
    struct ggml_tensor  * a,
    ...
    result->op = GGML_OP_REPLICATION_PAD2D;
    ggml_set_op_params_i32(result, 0, left);
    ggml_set_op_params_i32(result, 1, right);
    ggml_set_op_params_i32(result, 2, top);
    ggml_set_op_params_i32(result, 3, bottom);
...
struct ggml_tensor * ggml_reflection_pad2d(
    struct ggml_context * ctx,
    struct ggml_tensor  * a,
    int left, int right, int top, int bottom) {
    bool is_node = false;
    ...
    result->op = GGML_OP_REFLECTION_PAD2D;
    ggml_set_op_params_i32(result, 0, left);
    ggml_set_op_params_i32(result, 1, right);
    ggml_set_op_params_i32(result, 2, top);
    ggml_set_op_params_i32(result, 3, bottom);
...
struct ggml_tensor * ggml_deconv_pad2d(
    struct ggml_context * ctx,
    struct ggml_tensor  * a,
    int stride) {
    bool is_node = false;
    ...
    result->op = GGML_OP_DECONV_PAD2D;
    ggml_set_op_params_i32(result, 0, stride);
...
static void ggml_compute_forward_sin_cos_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    const int n  = ggml_nrows(src0);
    const int nc = src0->ne[0]; // W

    GGML_ASSERT( dst->nb[0] == sizeof(float));
    GGML_ASSERT(src0->nb[0] == sizeof(float));

    for (int i = 0; i < n; i++) {
        ggml_vec_sin_cos_f32(nc,
                (float *) ((char *) dst->data  + i*( dst->nb[1])),
                (float *) ((char *) src0->data + i*(src0->nb[1])));
    }
}

static void ggml_compute_forward_cos_sin_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward_sin_cos(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_sin_cos_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_cos_sin(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_cos_sin_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_cumsum_f32(
    ...
    if (dim == 0) {
        tensor_foreach_d0(dst) {
            sum = 0.0;
            offset = tensor_offset(src, 0/*i0*/, i1, i2, i3);
    ...
    if (dim == 1) {
        tensor_foreach_d1(dst) {
            sum = 0.0;
            offset = tensor_offset(src, i0, 0/*i1*/, i2, i3);
...
static void ggml_compute_forward_norm2_f32(
    ...
    if (dim == 0) {
        tensor_foreach_d0(src) {
            ...
            offset = tensor_offset(dst, 0 /*i0*/, i1, i2, i3);
    ...
    if (dim == 1) {
        tensor_foreach_d1(src) {
            ...
            offset = tensor_offset(dst, i0, 0/*i1*/, i2, i3);
            *(float *)((char *) dst->data + offset) = sqrtf(sum); // Save
        }
        return ;
    }
    ...
    if (dim == 2) {
        tensor_foreach_d2(src) {
            sum = 0.0;
            offset = tensor_offset(src, i0, i1, 0 /*i2*/, i3);
            for (int64_t i2 = 0; i2 < src->ne[2]; i2++) {
                x = *(float *) ((char *) src->data + offset);
                sum += x*x;
                offset += src->nb[2];
            } // i0
            offset = tensor_offset(dst, i0, i1, 0 /*i2*/, i3);
            *(float *)((char *) dst->data + offset) = sqrtf(sum); // Save
        }
        return ;
    }
    ...
    if (dim == 3) {
        tensor_foreach_d3(src) {
            sum = 0.0;
            offset = tensor_offset(src, i0, i1, i2, 0 /*i3*/);
            for (int64_t i3 = 0; i3 < src->ne[3]; i3++) {
                x = *(float *) ((char *) src->data + offset);
                sum += x*x;
                offset += src->nb[3];
            } // i0
            offset = tensor_offset(dst, i0, i1, i2, 0/*i3*/);
            *(float *)((char *) dst->data + offset) = sqrtf(sum); // Save
        }
        return ;
    }
...
// dell_xxxx
static void cpu_do_mean_ext_f32(const struct ggml_tensor* src, struct ggml_tensor* dst, 
    const int dim, const int64_t src_offset, const int64_t dst_offset)
{
    float mean, *p;
    ggml_float sum = 0.0;

    int64_t s_offset = src_offset;

    for (int64_t i = 0; i < src->ne[dim]; i++) {
        p = (float *)((char *)src->data + s_offset);
        sum += (ggml_float)(*p);
        s_offset += src->nb[dim];
    }
    mean = sum/src->ne[dim];

    *(float *)((char *)dst->data + dst_offset) = mean;
}

// dell_xxxx
static void ggml_compute_forward_mean_ext_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    ...
    if (dim == 0) {
        tensor_foreach_d0(src) {
            src_offset = tensor_offset(src, 0/*i0*/, i1, i2, i3);
            dst_offset = tensor_offset(dst, 0/*i0*/, i1, i2, i3);
            cpu_do_mean_ext_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
    ...
    if (dim == 1) {
        tensor_foreach_d1(src) {
            src_offset = tensor_offset(src, i0, 0 /*i1*/, i2, i3);
            dst_offset = tensor_offset(dst, i0, 0 /*i1*/, i2, i3);
            cpu_do_mean_ext_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
    ...
    if (dim == 2) {
        tensor_foreach_d2(src) {
            src_offset = tensor_offset(src, i0, i1, 0 /*i2*/, i3);
            dst_offset = tensor_offset(dst, i0, i1, 0 /*i2*/, i3);
            cpu_do_mean_ext_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
    ...
    if (dim == 3) {
        tensor_foreach_d3(src) {
            src_offset = tensor_offset(src, i0, i1, i2, 0 /*i3*/);
            dst_offset = tensor_offset(dst, i0, i1, i2, 0 /*i3*/);
            cpu_do_mean_ext_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
...
// dell_xxxx
static void cpu_do_corr_sum_f32(struct ggml_tensor* dst, 
    const struct ggml_tensor* f1,  const struct ggml_tensor* f2,
    const int64_t c, const int64_t start_i, const int64_t start_j)
{
    float f1_value, f2_value, sum;
    int64_t f1_offset, f2_offset, dst_offset;

    tensor_foreach_d2(dst) {
        sum = 0.0;

        for (int64_t i2 = 0; i2 < f1->ne[2]; i2++) {
            if (i0 + start_j < 0 || i1 + start_i < 0 || i0 + start_j >= f1->ne[0] || i1 + start_i >= f1->ne[1])
                continue;

            f1_offset = tensor_offset(f1, i0, i1, i2, i3);
            f2_offset = tensor_offset(f2, i0 + start_j, i1 + start_i, i2, i3);
            f1_value = *(float *)((char *)f1->data + f1_offset);
            f2_value = *(float *)((char *)f2->data + f2_offset);
            sum +=  f1_value * f2_value;
        }

        dst_offset = tensor_offset(dst, i0, i1, c/*i2*/, i3);
        *(float *)((char *)dst->data + dst_offset) = sum;
    }
}

// dell_xxxx
static void ggml_compute_forward_corr_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * f1 = dst->src[0];
    const struct ggml_tensor * f2 = dst->src[1];
    const int patch_size = ggml_get_op_params_i32(dst, 0);
    int64_t start_i, start_j;

    // Torch dst[B, C, H, W] = ?
    for (int64_t c = 0; c < dst->ne[2]; c++) {
        start_i = c / patch_size - patch_size/2; // - top_pad
        start_j = c % patch_size - patch_size/2; // - left_pad

        cpu_do_corr_sum_f32(dst, f1, f2, c, start_i, start_j);
    }
}

// dell_xxxx
static void ggml_compute_forward_global_attn_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    const int H1 = ggml_get_op_params_i32(dst, 0);
    const int W1 = ggml_get_op_params_i32(dst, 1);
    const int H2 = ggml_get_op_params_i32(dst, 2);
    const int W2 = ggml_get_op_params_i32(dst, 3);
    const int D2 = ggml_get_op_params_i32(dst, 4);
    ...
    float *p, *src_d;
    int64_t dst_offset, index, y_t, x_t;
    index = 0;
    src_d = src->data;
    for (int64_t i = 0; i < H1 * W1; i++) { // i1
        for (int64_t j = 0; j < H2 * W2; j++) { // i0
            y_t = i / W1 - j / W2 + D2;
            x_t = i % W1 - j % W2 + D2;
            dst_offset = tensor_offset(dst, j/*i0*/, i/*i1*/, 0, 0);
            p = (float *)((char *)dst->data + dst_offset);
            *p = (y_t >= -D2 && y_t <= D2 && x_t >= -D2 && x_t <= D2)? src_d[index++] : 0.0f;
        }
    }
}
...
static void ggml_compute_forward_mean_ext(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_mean_ext_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_max_f32(
    ...
    if (dim == 0) {
        tensor_foreach_d0(src) {
            s_offset = tensor_offset(src, 0/*i0*/, i1, i2, i3);
            p = (float *)((char *)src->data + s_offset);
            m = *p;
            for (int64_t i0 = 1; i0 < src->ne[0]; i0++) {
                s_offset += src->nb[0];
                p = (float *)((char *)src->data + s_offset);
                m = MAX(m, *p);
            }
            d_offset = tensor_offset(dst, 0/*i0*/, i1, i2, i3);
            *(float *)((char *)dst->data + d_offset) = m;
        }
        return;
    }
    ...
    if (dim == 1) {
        tensor_foreach_d1(src) {
            s_offset = tensor_offset(src, i0, 0 /*i1*/, i2, i3);
            p = (float *)((char *)src->data + s_offset);
            m = *p;
            for (int64_t i1 = 1; i1 < src->ne[1]; i1++) {
                s_offset += src->nb[1];
                p = (float *)((char *)src->data + s_offset);
                m = MAX(m, *p);
            }
            d_offset = tensor_offset(dst, i0, 0 /*i1*/, i2, i3);
            *(float *)((char *)dst->data + d_offset) = m;
        }
        return;
    }
    ...
    if (dim == 2) {
        tensor_foreach_d2(src) {
            s_offset = tensor_offset(src, i0, i1, 0 /*i2*/, i3);
            p = (float *)((char *)src->data + s_offset);
            m = *p;
            for (int64_t i2 = 1; i2 < src->ne[2]; i2++) {
                s_offset += src->nb[2];
                p = (float *)((char *)src->data + s_offset);
                m = MAX(m, *p);
            }
            d_offset = tensor_offset(dst, i0, i1, 0 /*i2*/, i3);
            *(float *)((char *)dst->data + d_offset) = m;
        }
        return;
    }
    ...
    if (dim == 3) {
        tensor_foreach_d3(src) {
            s_offset = tensor_offset(src, i0, i1, i2, 0 /*i3*/);
            p = (float *)((char *)src->data + s_offset);
            m = *p;
            for (int64_t i3 = 1; i3 < src->ne[3]; i3++) {
                s_offset += src->nb[3];
                p = (float *)((char *)src->data + s_offset);
                m = MAX(m, *p);
            }
            d_offset = tensor_offset(dst, i0, i1, i2, 0 /*i3*/);
            *(float *)((char *)dst->data + d_offset) = m;
        }
        return;
    }
...
static void ggml_compute_forward_max(
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_max_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
// dell_add
static void ggml_compute_forward_min_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
...
static void ggml_compute_forward_min(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_min_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_corr(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_corr_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_global_attn(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_global_attn_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static inline void get_f32_argmax(const struct ggml_tensor *src, int dim, int64_t start_offset, int64_t *index)
{
    float m, *p;
    ...
    *index = 0;
    m = -INFINITY;
    for (int64_t i = 0; i < src->ne[dim]; i++) {
        p = (float *)((char *)src->data + start_offset);
        if (*p > m) {
            *index = i;
            m = *p;
        }
        start_offset += src->nb[dim];
    }
}
...
static void ggml_compute_forward_argmax_ext_f32(
    ...
    if (dim == 0) {
        tensor_foreach_d0(dst) {
            ...
            get_f32_argmax(src, dim, s_offset, &s_index);
            *(float *)((char *)dst->data + d_offset) = (float)s_index;
        }
        return;
    }
...
// dell_xxxx
static void ggml_compute_forward_repeat_ext_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    GGML_UNUSED(params);
...
static void ggml_compute_forward_repeat_ext(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
...
static void ggml_compute_forward_cat_f32(
    ...
    const int32_t dim = ggml_get_op_params_i32(dst, 1);
    GGML_ASSERT(dim >= 0 && dim < 4);

    int dim_c = 0;
    float v;
    struct ggml_tensor *t;
    int64_t t_offset, d_offset;

    if (dim == 0) {
        for (int i = 0; i < n; i++) {
            t = dst->src[i];
            tensor_foreach_d0(t) {
                t_offset = tensor_offset(t, 0 /*i0*/, i1, i2, i3);
                d_offset = tensor_offset(dst, 0 + dim_c /*i0*/, i1, i2, i3);
    ...
    if (dim == 1) {
        for (int i = 0; i < n; i++) {
            t = dst->src[i];
            tensor_foreach_d1(t) {
                t_offset = tensor_offset(t, i0, 0 /*i1*/, i2, i3);
                d_offset = tensor_offset(dst, i0, 0 + dim_c /*i1*/, i2, i3);
    ...
    if (dim == 2) {
        for (int i = 0; i < n; i++) {
            t = dst->src[i];
            tensor_foreach_d2(t) {
                t_offset = tensor_offset(t, i0, i1, 0 /*i2*/, i3);
                d_offset = tensor_offset(dst, i0, i1, 0 + dim_c /*i2*/, i3);
    ...
    if (dim == 3) {
        for (int i = 0; i < n; i++) {
            t = dst->src[i];
            tensor_foreach_d3(t) {
                t_offset = tensor_offset(t, i0, i1, i2, 0 /*i3*/);
                d_offset = tensor_offset(dst, i0, i1, i2, 0 + dim_c /*i3*/);
...
static void ggml_compute_forward_cat(
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_cat_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
// dell_xxxx
static void cpu_do_norm_ext_f32(const struct ggml_tensor *src, struct ggml_tensor *dst, 
    const int dim, const int64_t src_offset, const int64_t dst_offset, const float eps)
{
    ggml_float sum, sum2;
    ...
    d_offset = dst_offset;
    for (int64_t i = 0; i < src->ne[dim]; i++) {
        sp = (float *)((char *)src->data + s_offset);
        v = *sp - mean;
        dp = (float *)((char *)dst->data + d_offset);
        *dp = v;
        sum2 += (ggml_float)(v*v);
...
static void ggml_compute_forward_norm_ext_f32(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    ...
    if (dim == 0) {
        tensor_foreach_d0(src) {
            src_offset = tensor_offset(src, 0/*i0*/, i1, i2, i3);
            dst_offset = tensor_offset(dst, 0/*i0*/, i1, i2, i3);
            cpu_do_norm_ext_f32(src, dst, dim, src_offset, dst_offset, eps);
        }
        return;
    }
...
static void ggml_compute_forward_norm_ext(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_norm_ext_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void cpu_do_softmax_f32(const struct ggml_tensor *src, struct ggml_tensor *dst, int dim, 
    ...
    for (int64_t i = 0; i < dst->ne[dim]; i++) {
        sp = (float *)((char *)src->data + s_offset);
        dp = (float *)((char *)dst->data + s_offset);
    ...
    d_offset = dst_offset;
    for (int64_t i = 0; i < dst->ne[dim]; i++) {
        p = (float *)((char *)dst->data + d_offset);
        *p = (*p)/sum;
...
static void ggml_compute_forward_softmax_f32(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    ...
    if (dim == 0) {
        tensor_foreach_d0(src) {
            ...
            cpu_do_softmax_f32(src, dst, dim, s_offset, d_offset);
        }
        return;
    }
...
static void ggml_compute_forward_softmax(
        const struct ggml_compute_params * params,
              struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_softmax_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_constant_f32_16(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    float value;
    memcpy(&value, (float *) dst->op_params + 0, sizeof(float));
...
static void ggml_compute_forward_constant(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_F16:
            {
                ggml_compute_forward_constant_f32_16(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_add_constant(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_F16:
            {
                ggml_compute_forward_add_constant_f32_16(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void cpu_do_interpolate_f32(const struct ggml_tensor *src, struct ggml_tensor *dst, int dim, 
    const int64_t src_offset, const int64_t dst_offset) {

    float sf, d, u, *x1, *x2, *y;
    int64_t s_1, s_2, s_offset1, s_offset2, d_offset;

    sf = (float)src->ne[dim]/(float)dst->ne[dim];
    for (int64_t i = 0; i < dst->ne[dim]; i++) {
        d = (float)(i + 0.5f)*sf - 0.5f;
        if (d < 0.001f) d = 0.001f;
        if (d > (float)dst->ne[dim] - 1.0f) d = (float)dst->ne[dim] - 1.0f;

        s_1 = (int64_t)d;
        s_2 = (s_1 + 1 < src->ne[dim])?s_1 + 1 : s_1;
        u = d - s_1;

        s_offset1 = src_offset + s_1 * src->nb[dim];
        s_offset2 = src_offset + s_2 * src->nb[dim];
        d_offset = dst_offset + i * dst->nb[dim];

        x1 = (float *)((char *)src->data + s_offset1);
        x2 = (float *)((char *)src->data + s_offset2);
        y = (float *)((char *)dst->data + d_offset);

        *y = (1.0f - u) * (*x1) + u*(*x2); // interpolate ...  more near x*, more weight !
    }
}
...
static void ggml_compute_forward_interpolate_f32(
    ...
    if (dim == 0) {
        tensor_foreach_d0(dst) {
            src_offset = tensor_offset(src, 0, i1, i2, i3);
            dst_offset = tensor_offset(dst, 0, i1, i2, i3);
            cpu_do_interpolate_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
    ...
    if (dim == 1) {
        tensor_foreach_d1(dst) {
            src_offset = tensor_offset(src, i0, 0, i2, i3);
            dst_offset = tensor_offset(dst, i0, 0, i2, i3);
            cpu_do_interpolate_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
    ...
    if (dim == 2) {
        tensor_foreach_d2(dst) {
            src_offset = tensor_offset(src, i0, i1, 0, i3);
            dst_offset = tensor_offset(dst, i0, i1, 0, i3);
            cpu_do_interpolate_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
    ...
    if (dim == 3) {
        tensor_foreach_d3(dst) {
            src_offset = tensor_offset(src, i0, i1, i2, 0);
            dst_offset = tensor_offset(dst, i0, i1, i2, 0);
            cpu_do_interpolate_f32(src, dst, dim, src_offset, dst_offset);
        }
        return;
    }
...
// dell_xxxx
static void ggml_compute_forward_grid_mesh_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    ...
    int64_t dst_offset;
    // dst shape: [2, W, H, B]
    tensor_foreach_d0(dst) { // i1 -- W, i2 -- H, i3 -- B
        dst_offset = tensor_offset(dst, 0/*i0 -- for x*/, i1, i2, i3);
        *(float *)((char *)dst->data + dst_offset) = (norm)? (float)i1/dst->ne[1] : (float)i1; // w;
        dst_offset = tensor_offset(dst, 1/*i0 -- for y*/, i1, i2, i3);
        *(float *)((char *)dst->data + dst_offset) = (norm)? (float)i2/dst->ne[2] : (float)i2; // h
    }
}
...
static void ggml_compute_forward_grid_sample_f32(
    ...
    const struct ggml_tensor * src = dst->src[0];
    const struct ggml_tensor * grid = dst->src[1];
    ...
    tensor_foreach_d3(dst) { // i0 -- W, i1 -- H, i2 -- C, i3 -- B
        for (int64_t i3 = 0; i3 < dst->ne[3]; i3++) {
            // grid[b, h, w, 0]
            g_offset = tensor_offset(grid, 0/*for x*/, i0/*Wout*/, i1 /*Hout*/, i3/*B*/);
            float x0 = *(float *)((char *)grid->data + g_offset);
            // grid[b, h, w, 1]
            g_offset = tensor_offset(grid, 1/*for y*/, i0/*Wout*/, i1 /*Hout*/, i3/*B*/);
            float y0 = *(float *)((char *)grid->data + g_offset);
...
// dell_xxxx
static void ggml_compute_forward_soft_splat_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    GGML_UNUSED(params);

    const struct ggml_tensor * src = dst->src[0];
    const struct ggml_tensor * flow = dst->src[1];

    GGML_ASSERT(src->type == GGML_TYPE_F32);
    GGML_ASSERT(dst->type == GGML_TYPE_F32);
    GGML_ASSERT(flow->type == GGML_TYPE_F32);

    const int H = src->ne[1]; // === dst->ne[1]
    const int W = src->ne[0]; // === dst->ne[0]
    int64_t src_offset, dst_offset, f_offset;

    // dst[b, c, h, w] = src[b, c, fx + mesh_x, fy + mesh_y]
    // flow[b, d, h, w]
    tensor_foreach_d3(dst) { // i0 -- W, i1 -- H, i2 -- C, i3 -- B
        for (int64_t i3 = 0; i3 < dst->ne[3]; i3++) {
            f_offset = tensor_offset(flow, i0, i2, 0 /*flow for x*/, i3);
            float fx = *(float *)((char *)flow->data + f_offset);
            f_offset = tensor_offset(flow, i0, i2, 1 /*flow for y*/, i3);
            float fy = *(float *)((char *)flow->data + f_offset);
...
// dell_xxxx
// flow -- [B, D, H, W]
static void ggml_compute_forward_eluer_motion_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    GGML_UNUSED(params);

    const struct ggml_tensor * flow = dst->src[0];
    GGML_ASSERT(dst->type == GGML_TYPE_F32);
    GGML_ASSERT(flow->type == GGML_TYPE_F32);

    const int n = ggml_get_op_params_i32(dst, 0);
    const int W = (int)dst->ne[0];
    const int H = (int)dst->ne[1];

    float d, f;
    int64_t d_offset, f_offset;
    // 1) Initialize
    tensor_foreach_d2(dst) {
        d_offset = tensor_offset(dst, i0 /*W*/, i1 /*H*/, 0 /*i2 -- C, 0 for x*/, i3);
        *(float *)((char *)dst->data + d_offset) = (float)i0;
        d_offset = tensor_offset(dst, i0 /*W*/, i1 /*H*/, 1 /*i2 -- C, 0 for y*/, i3);
        *(float *)((char *)dst->data + d_offset) = (float)i1;
    }
    // 2) Update
    for (int i = 0; i < n; i++) {
        tensor_foreach_d2(dst) {
            // update x
            d_offset = tensor_offset(dst, i0 /*W*/, i1 /*H*/, 0 /*i2 -- C, 0 for x*/, i3);
            d = *(float *)((char *)dst->data + d_offset);
            f_offset = tensor_offset(flow, i0 /*W*/, i1 /*H*/, 0 /*i2 -- C, 0 for x*/, i3);
            f = *(float *)((char *)flow->data + f_offset);
            d += f;
            if (d < 0.0f || d >= (float)(W - 1)) {
                d = (float)i0;
            }
            *(float *)((char *)dst->data + d_offset) = d;

            // update y
            d_offset = tensor_offset(dst, i0 /*W*/, i1 /*H*/, 1 /*i2 -- C, 1 for y*/, i3);
            d = *(float *)((char *)dst->data + d_offset);
            f_offset = tensor_offset(flow, i0 /*W*/, i1 /*H*/,1 /*i2 -- C, 1 for y*/, i3);
            f = *(float *)((char *)flow->data + f_offset);
            d += f;
            if (d < 0.0f || d >= (float)(H - 1)) {
                d = (float)i1;
            }
            *(float *)((char *)dst->data + d_offset) = d;
        }
    ...
    tensor_foreach_d2(dst) {
        d_offset = tensor_offset(dst, i0 /*W*/, i1 /*H*/, 0 /*i2 -- C, 0 for x*/, i3);
        d = *(float *)((char *)dst->data + d_offset);
        *(float *)((char *)dst->data + d_offset) = d - (float)i0;

        d_offset = tensor_offset(dst, i0 /*W*/, i1 /*H*/, 1 /*i2 -- C, 0 for y*/, i3);
        d = *(float *)((char *)dst->data + d_offset);
        *(float *)((char *)dst->data + d_offset) = d - (float)i1;
    }
}


// pixel shuffle
static void ggml_compute_forward_shuffle_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    ...
    // TODO: optimize
    int64_t s_offset, d_offset, s_0, s_1, s_2; // s_3

    // torch convert from src: src(B, C*R^2, H, W) to dst: dst(B, C, H*R, W*R)
    for (int64_t d_0 = 0; d_0 < dst->ne[0]; d_0++) { // W
        s_0 = d_0 / R;
        for (int64_t d_1 = 0; d_1 < dst->ne[1]; d_1++) { // H
            s_1 = d_1 / R;
            for (int64_t d_2 = 0; d_2 < dst->ne[2]; d_2++) { // C
                // s_c =  d_c * R * R + (d_h % R) * R + (d_w % R)
                s_2 = d_2 * R * R + (d_1 % R) * R + (d_0 % R); // src->channel !!!
                s_offset = tensor_offset(src, s_0, s_1, s_2, 0 /*s_3*/);
                d_offset = tensor_offset(dst, d_0, d_1, d_2, 0 /*d_3*/);
                for (int64_t d_3 = 0; d_3 < dst->ne[3]; d_3++) { // B
                    *(float *)((char *) dst->data + d_offset) = *(float *)((char *) src->data + s_offset);
                    s_offset += src->nb[3];
                    d_offset += dst->nb[3];
                }
...
// dell_xxxx
static void ggml_compute_forward_flip_f32_16(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    ...
    const int dim1 = ggml_get_op_params_i32(dst, 1);
    const int dim2 = ggml_get_op_params_i32(dst, 2);
    const int dim3 = ggml_get_op_params_i32(dst, 3);

    int64_t s_offset, d_offset, s_0, s_1, s_2, s_3;

    if (dst->type == GGML_TYPE_F32) {
        for (int64_t d_0 = 0; d_0 < dst->ne[0]; d_0++) {
            s_0 = (dim0 > 0)? dst->ne[0] - 1 - d_0 : d_0;
            for (int64_t d_1 = 0; d_1 < dst->ne[1]; d_1++) {
                s_1 = (dim1 > 0)? dst->ne[1] - 1 - d_1 : d_1;
                for (int64_t d_2 = 0; d_2 < dst->ne[2]; d_2++) {
                    s_2 = (dim2 > 0)? dst->ne[2] - 1 - d_2 : d_2;
                    // ------------------------------------------------------------
                    d_offset = tensor_offset(dst, d_0, d_1, d_2, 0 /*d_3*/);
                    for (int64_t d_3 = 0; d_3 < dst->ne[3]; d_3++) {
                        s_3 = (dim3 > 0)? dst->ne[3] - 1 - d_3 : d_3;
                        s_offset = tensor_offset(src, s_0, s_1, s_2, s_3);
                        const float * x = (float *)((char *) src->data + s_offset);
                              float * y = (float *)((char *) dst->data + d_offset);
                        *y = *x;
                        d_offset += dst->nb[3];
                    }
                }
            ...
            for (int64_t d_1 = 0; d_1 < dst->ne[1]; d_1++) {
                ...
                for (int64_t d_2 = 0; d_2 < dst->ne[2]; d_2++) {
                    ...
                    for (int64_t d_3 = 0; d_3 < dst->ne[3]; d_3++) {
                        s_3 = (dim3 > 0)? dst->ne[3] - 1 - d_3 : d_3;
                        s_offset = tensor_offset(src, s_0, s_1, s_2, s_3);
                        const ggml_fp16_t* x = (ggml_fp16_t *)((char *) src->data + s_offset);
                              ggml_fp16_t* y = (ggml_fp16_t *)((char *) dst->data + d_offset);
                        *y = *x;
                        d_offset += dst->nb[3];
                    }
                }
...
static void ggml_compute_forward_scatter_f32(
    ...
    const struct ggml_tensor *src0 = dst->src[0];
    const struct ggml_tensor *src1 = dst->src[1]; // index

    GGML_ASSERT(dst->type == GGML_TYPE_F32);
    GGML_ASSERT(src0->type == GGML_TYPE_F32);
    GGML_ASSERT(src1->type == GGML_TYPE_I32); // index
    ...
    if (dim == 0) {
        tensor_foreach_d0(dst) {
            offset = tensor_offset(dst, index[i1] /*i0*/, i1, i2, i3);
            *(float *)((char *)  dst->data + offset) = 1.0;
        }
        return;
    }
    ...
    if (dim == 1) {
        tensor_foreach_d1(dst) {
            offset = tensor_offset(dst, i0, index[i0] /*i1*/, i2, i3);
            *(float *)((char *)  dst->data + offset) = 1.0;
        }
        return;
    }
...
static void ggml_compute_forward_slice_scatter_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    GGML_UNUSED(params);

    const struct ggml_tensor * src0 = dst->src[0];
    const struct ggml_tensor * src1 = dst->src[1]; // The tensor to embed into input
    ...
    // GGML_TENSOR_UNARY_OP_LOCALS
    const int dim = ggml_get_op_params_i32(dst, 0);
    const int start = ggml_get_op_params_i32(dst, 1);
    int stop = ggml_get_op_params_i32(dst, 2);
    const int step = ggml_get_op_params_i32(dst, 3);
        ...
        for (int64_t i1 = 0; i1 < dst->ne[1]; i1++) { // H
            for (int64_t i2 = 0; i2 < dst->ne[2]; i2++) { // C
                for (int64_t i3 = 0; i3 < dst->ne[3]; i3++) { // B
                    int64_t index = tensor_offset(dst, i0, i1, i2, i3);
                    // i0*dst->nb[0] + i1*dst->nb[1] + i2*dst->nb[2] + i3*dst->nb[3]; // byte offset
                    const float * x = (float *)((char *) src0->data + index);
                          float * y = (float *)((char *)  dst->data + index);
                    *y = *x;
                }
            }
        }
    ...
    // embed src1 to dst
    // stop = MIN(stop, dst->ne[dim]); // limit stop 
    if (stop > (int)dst->ne[dim])
        stop = (int)dst->ne[dim];

    for (int64_t i0 = 0; i0 < src1->ne[0]; i0++) { // W
        const int64_t d_0 = (dim == 0)? start + i0 * step : i0;
        if (dim == 0 && (int)d_0 >= stop)
            break;
        for (int64_t i1 = 0; i1 < src1->ne[1]; i1++) { // H
            const int64_t d_1 = (dim == 1)? start + i1 * step : i1;
            if (dim == 1 && (int)d_1 >= stop)
                break;
            for (int64_t i2 = 0; i2 < src1->ne[2]; i2++) { // C
                const int64_t d_2 = (dim == 2)? start + i2 * step : i2;
                if (dim == 2 && (int)d_2 >= stop)
                    break;
                for (int64_t i3 = 0; i3 < src1->ne[3]; i3++) { // B
                    const int64_t d_3 = (dim == 3)? start + i3 * step : i3;
                    if (dim == 3 && (int)d_3 >= stop)
                        break;
                    const float * x = (float *)((char *) src1->data + i0*src1->nb[0] + i1*src1->nb[1] + i2*src1->nb[2] + i3*src1->nb[3]);
                          float * y = (float *)((char *) dst->data +  d_0*dst->nb[0] + d_1*dst->nb[1] + d_2*dst->nb[2] + d_3*dst->nb[3]);
                    *y = *x;
                }
            }
        }
...
// RFFT2: Real: (B, C, H, W) --> Complex: (B, C, H, (W/2)+1) --> Real: (B, 2*C, H, (W/2) + 1)
//IRFFT2: Real: (B, 2*C, H, (W/2) + 1) --> Complex: (B, C, H, (W/2)+1) --> Real: (B, C, H, W)
static void ggml_compute_forward_rfft2_f32(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    ...
    GGML_ASSERT(dst->ne[3] == B && dst->ne[2] == 2*C); // B, C
    GGML_ASSERT(sizeof(fftwf_complex) == 2*sizeof(float));
    ...
    fftwf_free(input_float);
    fftwf_free(output_complex);
}
...
static void ggml_compute_forward_irfft2_f32(
    ...
    void *point;
    fftwf_complex* input_complex = (fftwf_complex *) fftwf_malloc(sizeof(fftwf_complex) * H * H2W);
    ...
    fftwf_destroy_plan(plan);
    fftwf_free(input_complex);
    fftwf_free(output_float);
}
...
static void ggml_compute_forward_grid_mesh(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
...
static void ggml_compute_forward_grid_sample(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_grid_sample_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_soft_splat(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_soft_splat_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_eluer_motion(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_eluer_motion_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_shuffle(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_shuffle_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_flip(
    ...
    switch (src0->type) {
        ...
        case GGML_TYPE_F16:
            {
                ggml_compute_forward_flip_f32_16(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_scatter(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
...
static void ggml_compute_forward_slice_scatter(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_slice_scatter_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_rfft2(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_rfft2_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_irfft2(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_irfft2_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_replication_pad2d_f32(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    ...
    const int left = ggml_get_op_params_i32(dst, 0);
    // const int right  = ggml_get_op_params_i32(dst, 1);
    const int top  = ggml_get_op_params_i32(dst, 2);
    // const int bottom  = ggml_get_op_params_i32(dst, 3);
    // GGML_TENSOR_UNARY_OP_LOCALS
    ...
    for (int64_t d_0 = 0; d_0 < dst->ne[0]; d_0++) { // W
        {
            if (d_0 >= left && d_0 < src->ne[0] + left) { 
                s_0 = d_0 - left;
            } else {
                s_0 = (d_0 < left) ? 0 : src->ne[0] - 1; // Left or right region
            }
        }
        for (int64_t d_1 = 0; d_1 < dst->ne[1]; d_1++) { // H
            {
                if (d_1 >= top && d_1 < src->ne[1] + top) {
                    s_1 = d_1 - top;
                } else {
...
// dell_xxxx
static void ggml_compute_forward_reflection_pad2d_f32(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    ...
    for (int64_t d_0 = 0; d_0 < dst->ne[0]; d_0++) { // W
        {
            if (d_0 >= left && d_0 < src->ne[0] + left) { 
                ...
                s_0 = (d_0 < left) ? left - d_0 : 2*(src->ne[0] - 1) - d_0; // Left or right region
            }
        }
        for (int64_t d_1 = 0; d_1 < dst->ne[1]; d_1++) { // H
            {
...
// dell_xxxx
static void ggml_compute_forward_deconv_pad2d_f32(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst) {
    GGML_UNUSED(params);
    const struct ggml_tensor * src = dst->src[0];
    ...
    // TODO: optimize
    int64_t s_offset, d_offset;
    for (int64_t s_0 = 0; s_0 < src->ne[0]; s_0++) { // W
        for (int64_t s_1 = 0; s_1 < src->ne[1]; s_1++) { // H
            for (int64_t s_2 = 0; s_2 < src->ne[2]; s_2++) {
...
static void ggml_compute_forward_replication_pad2d(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_replication_pad2d_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_reflection_pad2d(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_reflection_pad2d_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward_deconv_pad2d(
    const struct ggml_compute_params * params,
    struct ggml_tensor * dst) {
    ...
    switch (src0->type) {
        case GGML_TYPE_F32:
            {
                ggml_compute_forward_deconv_pad2d_f32(params, dst);
            } break;
        default:
            {
                GGML_ABORT("fatal error");
...
static void ggml_compute_forward(struct ggml_compute_params * params, struct ggml_tensor * tensor) {
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_SIN_COS:
            {
                ggml_compute_forward_sin_cos(params, tensor);
            } break;
        case GGML_OP_COS_SIN:
            {
                ggml_compute_forward_cos_sin(params, tensor);
            } break;
        case GGML_OP_SUM:
            {
        ...
        case GGML_OP_MEAN_EXT:
            {
                ggml_compute_forward_mean_ext(params, tensor);
            } break;
        case GGML_OP_MAX:
            {
                ggml_compute_forward_max(params, tensor);
            } break;
        case GGML_OP_MIN:
            {
                ggml_compute_forward_min(params, tensor);
            } break;
        case GGML_OP_CORR:
            {
                ggml_compute_forward_corr(params, tensor);
            } break;
        case GGML_OP_GLOBAL_ATTN:
            {
                ggml_compute_forward_global_attn(params, tensor);
            } break;
        case GGML_OP_ARGMAX:
            {
        ...
        case GGML_OP_REPEAT_EXT:
            {
                ggml_compute_forward_repeat_ext(params, tensor);
            } break;
        case GGML_OP_CONCAT:
            {
        ...
        case GGML_OP_CAT:
            {
                ggml_compute_forward_cat(params, tensor);
            } break;
        ...
        case GGML_OP_NORM_EXT:
            {
                ggml_compute_forward_norm_ext(params, tensor);
            } break;
        ...
        case GGML_OP_SOFTMAX:
            {
                ggml_compute_forward_softmax(params, tensor);
            } break;
        ...
        case GGML_OP_CONSTANT: 
            {
                ggml_compute_forward_constant(params, tensor);
            } break;
        case GGML_OP_ADD_CONSTANT: 
            {
                ggml_compute_forward_add_constant(params, tensor);
            } break;
        ...
        case GGML_OP_GRID_MESH:
            {
                ggml_compute_forward_grid_mesh(params, tensor);
            } break;
        case GGML_OP_GRID_SAMPLE:
            {
                ggml_compute_forward_grid_sample(params, tensor);
            } break;
        case GGML_OP_SOFT_SPLAT:
            {
                ggml_compute_forward_soft_splat(params, tensor);
            } break;
        case GGML_OP_EULER_MOTION:
            {
                ggml_compute_forward_eluer_motion(params, tensor);
            } break;
        case GGML_OP_SHUFFLE:
            {
                ggml_compute_forward_shuffle(params, tensor);
            } break;
        case GGML_OP_FLIP:
            {
        ...
        case GGML_OP_SLICE_SCATTER:
            {
                ggml_compute_forward_slice_scatter(params, tensor);
            } break;
        case GGML_OP_RFFT2:
            {
                ggml_compute_forward_rfft2(params, tensor);
            } break;
        case GGML_OP_IRFFT2:
            {
                ggml_compute_forward_irfft2(params, tensor);
            } break;
        case GGML_OP_PAD:
            {
        ...
        case GGML_OP_REPLICATION_PAD2D:
            {
                ggml_compute_forward_replication_pad2d(params, tensor);
            } break;
        case GGML_OP_REFLECTION_PAD2D:
            {
                ggml_compute_forward_reflection_pad2d(params, tensor);
            } break;
        case GGML_OP_DECONV_PAD2D:
            {
                ggml_compute_forward_deconv_pad2d(params, tensor);
            } break;
        case GGML_OP_ARANGE:
            {
...
struct ggml_tensor * ggml_graph_leaf(struct ggml_cgraph * cgraph, int i) {
    if (i < 0) {
        ...
        return cgraph->nodes[cgraph->n_leafs + i];
    }
...
struct gguf_context * gguf_init_from_file(const char * fname, struct gguf_init_params params) {
    ...
    if (params.ctx != NULL) {
        ...
        *params.ctx = ggml_init(pdata);
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            // Fixed bug: 
            // n_dims = 0, name = decoder.layers.0.shuf.conv.1.num_batches_tracked
            if (ctx->infos[i].n_dims == 0) { // Fixed
                ctx->infos[i].n_dims = 1;
                ctx->infos[i].ne[0] = 1;
            }
