...
struct Encoder : GGMLNetwork {
    ...
    Encoder() {
        ...
        // syslog_info("---------------------- block_in = %d", block_in); // 512
    ...
    void create_weight_tensors(struct ggml_context* ctx) {
        ...
        conv_in_b = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, ch); // got [128, 1, 1, 1], expected [512, 1, 1, 1]

        norm_out_w = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, block_in);
        norm_out_b = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, block_in); // got [512, 1, 1, 1], expected [128, 1, 1, 1]
    ...
    void setup_weight_names(const char* prefix) {
        ...
        for (int i = 0; i < len_mults; i++) {
            for (int j = 0; j < num_res_blocks; j++) {
                // down_block[i][j].setup_weight_names(
                //     prefix + "down." + std::to_string(i) + ".block." + std::to_string(j) + ".");
                snprintf(s, sizeof(s), "%sdown.%d.block.%d.", prefix, i, j);
                down_block[i][j].setup_weight_names(s);
            }
            if (i != len_mults - 1) {
                // down_sample[i].setup_weight_names(prefix + "down." + std::to_string(i) + ".downsample.");
...
struct Decoder : GGMLNetwork {
    ...
    void setup_weight_names(const char* prefix) {
        ...
        // mid.block_1.setup_weight_names(prefix + "mid.block_1.");
        snprintf(s, sizeof(s), "%s%s", prefix, "mid.block_1.");
        mid.block_1.setup_weight_names(s);

        // mid.attn_1.setup_weight_names(prefix + "mid.attn_1.");
        snprintf(s, sizeof(s), "%s%s", prefix, "mid.attn_1.");
        mid.attn_1.setup_weight_names(s);

        // mid.block_2.setup_weight_names(prefix + "mid.block_2.");
        snprintf(s, sizeof(s), "%s%s", prefix, "mid.block_2.");
        mid.block_2.setup_weight_names(s);
        ...
        for (int i = len_mults - 1; i >= 0; i--) {
            for (int j = 0; j < num_res_blocks + 1; j++) {
                // up_blocks[i][j].setup_weight_names(prefix + "up." + std::to_string(i) + ".block." + std::to_string(j) + ".");
                snprintf(s, sizeof(s), "%sup.%d.block.%d.", prefix, i, j);
                up_blocks[i][j].setup_weight_names(s);
            }
            if (i != 0) {
                // up_samples[i - 1].setup_weight_names(prefix + "up." + std::to_string(i) + ".upsample.");
...
struct AutoEncoderKL {
    ...
    struct ggml_tensor* decode(struct ggml_context* ctx0, struct ggml_tensor* z) {
        ...
        struct ggml_tensor* argv[1];
        argv[0] = h;

        h = decoder.forward(ctx0, 1, argv);
    ...
    struct ggml_tensor* encode(struct ggml_context* ctx0, struct ggml_tensor* x) {
        ...
        auto h = encoder.forward(ctx0, 1, argv);  // [N, 2*z_channels, h/8, w/8]
        // quant_conv
