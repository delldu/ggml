...
#include "vocab.h"
    ...
    std::u32string bpe(const std::u32string& token)
    {
        ...
        while (true) {
            auto min_pair_iter = std::min_element(pairs.begin(), pairs.end(),
...
struct CLIPMLP {
    ...
    {
        use_gelu = (d_model == 1024 || d_model == 1280)? true : false;
...
struct CLIPLayer {
    ...
    {
        // f32 [   768,    77,     1,     1], x
        x = ggml_add(ctx, x, self_attn.forward(ctx, layer_norm1.forward(ctx, x), true /*mask*/));
...
struct CLIPEncoder {
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, ggml_tensor_t* x, int clip_skip = -1)
    {
...
struct CLIPEmbeddings {
    ...
    ggml_tensor_t* token_embedding_weight;
    ggml_tensor_t* position_embedding_weight;
    ...
    void create_weight_tensors(ggml_context_t* ctx)
    {
        ...
        // position_embedding_weight = ggml_new_tensor_2d(ctx, GGML_TYPE_Q8_0, embed_dim, num_positions); // [768, 77, 1, 1]
    }
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, ggml_tensor_t* input_large_14)
    {
        ...
        token_embedding = ggml_reshape_3d(
            ctx, token_embedding, token_embedding->ne[0], token_embedding->ne[1], token_embedding->ne[3]);
...
struct CLIPTextModel {
    ...
    void create_weight_tensors(ggml_context_t* ctx)
    {
        ...
        if (version == OPEN_CLIP_VIT_BIGG_14) {
            text_projection
                = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, projection_dim, hidden_size); // OPEN_CLIP_VIT_BIGG_14
        }
...
struct TextEncoder : GGMLNetwork {
    ...
    void create_input_tensors(int argc, TENSOR* argv[], ggml_context_t* ctx, ggml_tensor_t* x[])
    {
        char input_name[64];
        for (int i = 0; i < argc; i++) {
            ...
            x[i] = ggml_new_tensor_4d(ctx, GGML_TYPE_I32 /*I32*/, (int64_t)argv[i]->width, (int64_t)argv[i]->height,
    ...
    // inputs must be GGML_TYPE_I32, override !!!
    void setup_input_values(int argc, TENSOR* argv[], bool is_cpu_backend, ggml_tensor_t* x[])
    {
        std::vector<int> temp;
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, int argc, ggml_tensor_t* argv[2])
    {
        ggml_tensor_t* input_large_14 = argv[0]; // i32 [    77,     1,     1,     1], net.input_0
        ggml_tensor_t* input_bigg_14 = argv[1]; // i32 [    77,     1,     1,     1], net.input_1
        ...
        auto hidden_states = text_model.forward(ctx, input_large_14, max_token_idx, return_pooled);
        hidden_states = ggml_reshape_4d(
            ctx, hidden_states, hidden_states->ne[0], hidden_states->ne[1], hidden_states->ne[2], hidden_states->ne[3]);
        ...
        hidden_states2 = ggml_reshape_4d(ctx, hidden_states2, hidden_states2->ne[0], hidden_states2->ne[1],
            hidden_states2->ne[2], hidden_states2->ne[3]);
