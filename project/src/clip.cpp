...
#define CHUNK_LEN77 77
    ...
    for (int i = 0; i < chunk_count; i++) {
        ...
        if (input_large_14) {
            for (int j = 0; j < CHUNK_LEN77; j++) {
                input_large_14->data[j] = (float)chunk_tokens[j];
            }
        } else {
        ...
        if (input_bigg_14) {
            for (int j = 0; j < CHUNK_LEN77; j++) {
                input_bigg_14->data[j] = (float)chunk_tokens[j];
            }
        } else {
    ...
    TENSOR* condition_latent
        = tensor_create(cond_latent[0]->batch, chunk_count, cond_latent[0]->height, cond_latent[0]->width);
    if (condition_latent) {
        for (int i = 0; i < chunk_count; i++) {
