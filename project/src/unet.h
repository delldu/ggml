...
struct GEGLU {
    ...
    void create_weight_tensors(ggml_context_t* ctx, ggml_type wtype = GGML_TYPE_F16 /*GGML_TYPE_Q8_0*/)
    {
...
struct FeedForward {
    ...
    void create_weight_tensors(ggml_context_t* ctx)
    {
        ...
        net_2.has_bias = true;
        net_2.create_weight_tensors(ctx, GGML_TYPE_F16 /*GGML_TYPE_Q8_0*/);
    }
...
struct CrossAttention {
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, ggml_tensor_t* x, ggml_tensor_t* context)
    {
        int64_t n = x->ne[2];
...
struct ResBlock {
    ...
    void create_weight_tensors(ggml_context_t* ctx)
    {
        ...
        if (!skip_t_emb) { // Linear
            ...
            emb_layer_1.has_bias = true;
            emb_layer_1.create_weight_tensors(ctx); // GGML_TYPE_F16
        }
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, ggml_tensor_t* x, ggml_tensor_t* emb = NULL)
    {
        if (emb == NULL) {
...
// ggml_tensor_t* ggml_nn_timestep_embedding(
//     ggml_context_t* ctx,
//     ggml_tensor_t* timesteps,
//     int dim,
...
struct UNetModel : GGMLNetwork {
    ...
    void create_weight_tensors(ggml_context_t* ctx)
    {
        ...
        out_2.padding = { 1, 1 };
        out_2.create_weight_tensors(ctx, GGML_TYPE_F16);
    }
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, int argc, ggml_tensor_t* argv[])
    {
        ...
        ggml_tensor_t* timesteps = argv[1];
        ggml_tensor_t* cond_latent = argv[2];
        ggml_tensor_t* cond_pooled = argv[3];
        ggml_tensor_t* controls_0 = argv[4];
        ggml_tensor_t* controls_1 = argv[5];
        ggml_tensor_t* controls_2 = argv[6];
        ggml_tensor_t* controls_3 = argv[7];

        // x --  f32 [96, 96, 4, 1], net.input_0
        // cond_latent -- f32 [2048, 77, 1, 1], net.input_2
        // cond_pooled -- f32 [2816, 1, 1, 1], net.input_3
        if (cond_latent != NULL && cond_latent->ne[2] != x->ne[3]) {
            cond_latent = ggml_repeat(ctx, cond_latent,
                ggml_new_tensor_3d(ctx, GGML_TYPE_F32, cond_latent->ne[0], cond_latent->ne[1], x->ne[3]));
        }
        if (cond_pooled != NULL && cond_pooled->ne[1] != x->ne[3]) {
            cond_pooled = ggml_repeat(ctx, cond_pooled,
                ggml_new_tensor_2d(ctx, GGML_TYPE_F32, cond_pooled->ne[0], x->ne[3]));
        }

        auto t_emb = ggml_timestep_embedding(ctx, timesteps, model_channels, 10000 /*max_period*/);
        ...
        if (cond_pooled != NULL) {
            ...
            label_emb = label_emb_0_2.forward(ctx, label_emb);
            emb = ggml_add(ctx, emb, label_emb);
        }

        // input_blocks
        std::vector<ggml_tensor_t*> hs;

        // input block 0
        auto h = input_blocks_0_0.forward(ctx, x); // f32 [96, 96, 320, 1]
        ...
        // ----------------------------------------------------------------------------------------------
        // i == 0
        h = input_blocks_1_0.forward(ctx, h, emb); // f32 [128, 64, 320, 1],
        hs.push_back(h);
        h = input_blocks_2_0.forward(ctx, h, emb); // f32 [128, 64, 320, 1],
        hs.push_back(h);
        h = input_blocks_3_0.forward(ctx, h); // DownSampleBlock // f32 [ 64, 32, 320, 1],
        if (controls_0 != NULL) {
            // f32 [ 64, 32, 320, 1], net.input_4, controls_0
            h = ggml_add(ctx, h, controls_0);
        }
        hs.push_back(h);

        // i == 1
        h = input_blocks_4_0.forward(ctx, h, emb); // f32 [ 64, 32, 640, 1],
        hs.push_back(h);
        h = input_blocks_5_0.forward(ctx, h, emb); // f32 [ 64, 32, 640, 1],
        if (controls_1 != NULL) {
            // f32 [ 64, 32, 640, 1], net.input_5 -- controls_1
            h = ggml_add(ctx, h, controls_1);
        }
        hs.push_back(h);
        h = input_blocks_6_0.forward(ctx, h); // DownSampleBlock, f32 [ 32, 16, 640, 1],
        hs.push_back(h);

        // i == 2
        h = input_blocks_7_0.forward(ctx, h, emb); // ResBlock
        h = input_blocks_7_1.forward(ctx, h, cond_latent); // SpatialTransformer , f32  32, 16, 1280, 1],
        hs.push_back(h);
        h = input_blocks_8_0.forward(ctx, h, emb); // ResBlock
        h = input_blocks_8_1.forward(ctx, h, cond_latent); // SpatialTransformer, f32 [ 32, 16, 1280, 1],
        if (controls_2 != NULL) {
            // f32 [ 32, 16, 1280, 1], net.input_6, controls_2
        ...
        // middle_block
        // ------------------------------------------------------------------------------------------------
        h = middle_block_0.forward(ctx, h, emb); // f32 [ 32, 16, 1280, 1],
        h = middle_block_1.forward(ctx, h, cond_latent); // f32 [ 32, 16, 1280, 1],
        h = middle_block_2.forward(ctx, h, emb); // f32 [ 32, 16, 1280, 1],
        if (controls_3 != NULL) {
            // f32 [ 32, 16, 1280, 1], net.input_7, controls_3
        ...
        h = output_blocks_0_1.forward(ctx, h, cond_latent); // f32 [24, 24, 1280, 1]

        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_1_0.forward(ctx, h, emb); // output_blocks_1_0
        h = output_blocks_1_1.forward(ctx, h, cond_latent); // f32 [24, 24, 1280, 1],

        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_2_0.forward(ctx, h, emb); // output_blocks_2_0
        h = output_blocks_2_1.forward(ctx, h, cond_latent);
        h = output_blocks_2_2.forward(ctx, h); // f32 [48, 48, 1280, 1]

        // output_blocks case i == 1
        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_3_0.forward(ctx, h, emb); // output_blocks_3_0
        h = output_blocks_3_1.forward(ctx, h, cond_latent); // f32 [48, 48, 640, 1]

        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_4_0.forward(ctx, h, emb); // output_blocks_4_0
        h = output_blocks_4_1.forward(ctx, h, cond_latent); // f32 [48, 48, 640, 1]

        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_5_0.forward(ctx, h, emb); // output_blocks_5_0
        h = output_blocks_5_1.forward(ctx, h, cond_latent);
        h = output_blocks_5_2.forward(ctx, h); // f32 [96, 96, 640, 1]

        // output_blocks case i == 0
        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_6_0.forward(ctx, h, emb); // f32 [96, 96, 320, 1]

        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_7_0.forward(ctx, h, emb); // f32 [96, 96, 320, 1]

        h_skip = hs.back(); hs.pop_back();
        h = ggml_concat(ctx, h, h_skip, 2);
        h = output_blocks_8_0.forward(ctx, h, emb); // f32 [96, 96, 320, 1]

        // out
        h = out_0.forward(ctx, h); // f32 [96, 96, 320, 1]
        h = ggml_silu_inplace(ctx, h);
        h = out_2.forward(ctx, h); // f32 [96, 96, 4, 1]
