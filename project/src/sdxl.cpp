...
static void euler_sample(UNetModel* unet,
    TENSOR* x, // image prompt
    TENSOR* positive_latent, TENSOR* positive_pooled, TENSOR* negative_latent, TENSOR* negative_pooled,
    float config_scale, // text prompt
    TENSOR* controls_0, TENSOR* controls_1, TENSOR* controls_2, TENSOR* controls_3, const std::vector<float>& sigmas,
    RNG* rng, Denoiser* denoiser);

static TENSOR* denoise_model(UNetModel* unet, TENSOR* positive_latent, TENSOR* positive_pooled, TENSOR* negative_latent,
    TENSOR* negative_pooled,
    float config_scale, // text prompt
    TENSOR* controls_0, TENSOR* controls_1, TENSOR* controls_2, TENSOR* controls_3, Denoiser* denoiser, TENSOR* input,
...
static std::vector<TENSOR*> get_controls(ModelConfig* config)
{
    ...
    for (int i = 0; i < 4; i++) {
        if (! tensor_valid(res[i])) {
            syslog_debug("Got null controls.");
            continue;
        }
...
int image2image(ModelConfig* config)
{
    ...
    // -----------------------------------------------------------------------------------------
    {
        ...
        euler_sample(&unet, noised_latent, positive_latent, positive_pooled, negative_latent, negative_pooled,
            config->config_scale, controls[0], controls[1], controls[2], controls[3], config->sigmas, &(config->rng),
            &(config->denoiser));
        unet.stop_engine();
    ...
    RNG* rng, Denoiser* denoiser)
{
    ...
    case EULER_A: {
        ...
        for (int i = 0; i < steps; i++) {
            ...
            TENSOR* noised_output = denoise_model(unet, positive_latent, positive_pooled, negative_latent,
                negative_pooled, config_scale, // text prompt
                controls_0, controls_1, controls_2, controls_3, denoiser, x, sigma, i + 1 /* step */);
            ...
            float sigma_up = std::min(sigmas[i + 1],
                std::sqrt(sigmas[i + 1] * sigmas[i + 1] * (sigmas[i] * sigmas[i] - sigmas[i + 1] * sigmas[i + 1])
                    / (sigmas[i] * sigmas[i])));
    ...
    float sigma, int step)
{
    ...
    CHECK_TENSOR(noised_output); // noised_output = 0
    ...
    // positive unet->forward ...
    // -----------------------------------------------------------------------------------------------------------------
    TENSOR* positive_output = unet_forward(unet, 
        noised_input, timesteps, positive_latent, positive_pooled, controls_0, controls_1, controls_2, controls_3);
    CHECK_TENSOR(positive_output);
    // -----------------------------------------------------------------------------------------------------------------

    // negative unet->forward ...
    // -----------------------------------------------------------------------------------------------------------------
    TENSOR* negative_output = unet_forward(unet, 
        noised_input, timesteps, negative_latent, negative_pooled, controls_0, controls_1, controls_2, controls_3);
    ...
    // update noised_output
    {
        ...
        for (int i = 0; i < n; i++) {
            latent_result
                = negative_output->data[i] + config_scale * (positive_output->data[i] - negative_output->data[i]);
