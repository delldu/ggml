...
static int set_input_tile(TENSOR* input, int start_row, int start_col, TENSOR* input_tile);
...
TENSOR* vae_decode(AutoEncoderKL* vae, TENSOR* latent)
{
    ...
    for (int start_row = 0; start_row < latent->height && (!last_row); start_row += non_tile_overlap) {
        ...
        for (int start_col = 0; start_col < latent->width && (!last_col); start_col += non_tile_overlap) {
            ...
            CHECK_POINT(set_input_tile(latent, start_row, start_col, input_tile) == RET_OK);
...
static int set_input_tile(TENSOR* input, int start_row, int start_col, TENSOR* input_tile)
{
    ...
    // input[:, :, start_row: start_tow + tile_size, start_col: start_col + tile_size] 
    ...
    for (int b = 0; b < output->batch; b++) {
        for (int c = 0; c < output->chan; c++) {
            for (int i = 0; i < tile_size; i++) {
                ...
                const float i_f_1
                    = (start_row < (output->height - tile_size)) ? float(tile_size - i) / float(overlap) : 1.0;
                const float i_f = std::min(std::min(i_f_0, i_f_1), 1.0f);

                for (int j = 0; j < tile_size; j++) {
                    const float j_f_0 = (start_col > 0) ? float(j) / float(overlap) : 1.0;
                    const float j_f_1
                        = (start_col < (output->width - tile_size)) ? float(tile_size - j) / float(overlap) : 1.0;
