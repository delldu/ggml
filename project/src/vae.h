...
struct DownSample {
    ...
    ggml_tensor_t* op_w; // [out_channels, channels, 3, 3]
    ggml_tensor_t* op_b; // [out_channels,]
...
struct UpSample {
    ...
    // conv2d params
    ggml_tensor_t* conv_w; // [out_channels, channels, 3, 3]
    ggml_tensor_t* conv_b; // [out_channels,]
...
struct ResnetBlock {
    ...
    // network params
    ggml_tensor_t* norm1_w; // [in_channels, ]
    ggml_tensor_t* norm1_b; // [in_channels, ]

    ggml_tensor_t* conv1_w; // [out_channels, in_channels, 3, 3]
    ggml_tensor_t* conv1_b; // [out_channels, ]

    ggml_tensor_t* norm2_w; // [out_channels, ]
    ggml_tensor_t* norm2_b; // [out_channels, ]

    ggml_tensor_t* conv2_w; // [out_channels, out_channels, 3, 3]
    ggml_tensor_t* conv2_b; // [out_channels, ]

    // nin_shortcut, only if out_channels != in_channels
    ggml_tensor_t* nin_shortcut_w; // [out_channels, in_channels, 1, 1]
    ggml_tensor_t* nin_shortcut_b; // [out_channels, ]
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, ggml_tensor_t* z)
    {
        // z: [N, in_channels, h, w]
...
struct AttnBlock {
    ...
    // group norm
    ggml_tensor_t* norm_w; // [in_channels,]
    ggml_tensor_t* norm_b; // [in_channels,]

    // q/k/v
    ggml_tensor_t* q_w; // [in_channels, in_channels, 1, 1]
    ggml_tensor_t* q_b; // [in_channels,]
    ggml_tensor_t* k_w; // [in_channels, in_channels, 1, 1]
    ggml_tensor_t* k_b; // [in_channels,]
    ggml_tensor_t* v_w; // [in_channels, in_channels, 1, 1]
    ggml_tensor_t* v_b; // [in_channels,]

    // proj_out
    ggml_tensor_t* proj_out_w; // [in_channels, in_channels, 1, 1]
    ggml_tensor_t* proj_out_b; // [in_channels,]
...
struct Encoder {
    ...
    ggml_tensor_t* conv_in_w; // [ch, in_channels, 3, 3]
    ggml_tensor_t* conv_in_b; // [ch, ]
    ...
    ggml_tensor_t* norm_out_w; // [block_in, ]
    ggml_tensor_t* norm_out_b; // [block_in, ]

    ggml_tensor_t* conv_out_w; // [embed_dim*2, block_in, 3, 3]
    ggml_tensor_t* conv_out_b; // [embed_dim*2, ]
...
struct Decoder {
    ...
    ggml_tensor_t* conv_in_w; // [block_in, z_channels, 3, 3]
    ggml_tensor_t* conv_in_b; // [block_in, ]
    ...
    ggml_tensor_t* norm_out_w; // [ch *  ch_mult[0], ]
    ggml_tensor_t* norm_out_b; // [ch *  ch_mult[0], ]

    ggml_tensor_t* conv_out_w; // [out_ch, ch *  ch_mult[0], 3, 3]
    ggml_tensor_t* conv_out_b; // [out_ch, ]
...
struct AutoEncoderKL : GGMLNetwork {
    ...
    ggml_tensor_t* quant_conv_w; // [2*embed_dim, 2*z_channels, 1, 1]
    ggml_tensor_t* quant_conv_b; // [2*embed_dim, ]

    ggml_tensor_t* post_quant_conv_w; // [z_channels, embed_dim, 1, 1]
    ggml_tensor_t* post_quant_conv_b; // [z_channels, ]
    ...
    ggml_tensor_t* forward(ggml_context_t* ctx, int argc, ggml_tensor_t* argv[])
    {
        if (encode_flag) {
            ...
            // quant_conv
            h = ggml_nn_conv_2d(ctx, h, quant_conv_w, quant_conv_b, 1, 1, 0, 0, 1, 1);
        ...
        // decode ...
        auto z = argv[0];
        auto h = ggml_nn_conv_2d(ctx, z, post_quant_conv_w, post_quant_conv_b, 1, 1, 0, 0, 1, 1);
